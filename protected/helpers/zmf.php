...
class zmf {
    ...
    public static function uploadDirs($ctime = '', $base = 'site', $type = 'posts', $return = '') {
        ...
        foreach ($sizes as $size) {
            $dir[$size] = $baseUrl . $type . '/' . $size . '/' . $_extra;
        }
        if (!empty($return)) {
            $dir = $dir[$return];
        }
    ...
    public static function imgurl($ctime, $filepath, $imgtype, $type = 'scenic') {
        $dir = self::uploadDirs($ctime, 'site', $type, $imgtype);
        return $dir . '/' . $filepath;
    }
        ...
        foreach ($match[1] as $key => $val) {
            ...
            if ($_status || $uid == Yii::app()->user->id) {
                ...
                if ($src) {
                    ...
                    $url = $src['filePath'];
                    $imgurl = self::imgurl($src['cTime'], $url, $size, $src['classify']);
                    if ($_width != '' AND $_width > 0 AND $_width < $size) {
                        $rate = $_width / $size;
                        $width = floor($src['width'] * $rate) . 'px';
                        $height = floor($src['height'] * $rate) . 'px';
                    } else {
                        $width = $src['width'] . 'px';
                    ...
                    if ($lazyload) {
                        ...
                        $imgurl = "<img src='{$imgurl}' alt='" . $altTitle . "' data='" . $match[1][$key] . "' class='thumbnail img-responsive'/>";
                    }
    ...
    public static function filterContent($content, &$arr_attachids) {
        ...
        $content = strip_tags($content, '<b><strong><em><span><a><p><u><i><br><br/>');
