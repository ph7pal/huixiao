...
class Link extends CActiveRecord {
    ...
    public function allLinks($limit = 10) {
        $sql = "SELECT title,url,attachid FROM {{link}} WHERE status=1 ORDER BY `order` LIMIT {$limit}";
        $items = Yii::app()->db->createCommand($sql)->queryAll();
        if (!empty($items)) {
            foreach ($items as $k => $it) {
                if ($it['attachid'] > 0) {
                    $attachinfo = Attachments::getOne($it['attachid']);
                    if ($attachinfo) {
                        $it['imgurl'] = zmf::imgurl($attachinfo['logid'], $attachinfo['filePath'], 124, $attachinfo['classify']);
                    } else {
                        $it['imgurl'] = '';
                    }
                } else {
